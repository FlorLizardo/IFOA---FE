...
function dice() {
	let casuali = Math.floor(Math.random() * 6) + 1;
	return casuali;
}
...
console.log("Il maggiore è: ", whoIsBigger(9, 7));
...
console.log("Esercizio 4: ", deleteOne("Ciao a tutti", false));
console.log("Esercizio 4: ", deleteOne("Ciao a tutti", true));
...
function onlyLetters(str) {
  ...
  const number = stringa.filter((el) => isNaN(el));
...
console.log("Esercizio 6: ", isThisAnEmail("email@email.com"));
console.log("Esercizio 6: ", isThisAnEmail("email.com"));
...
function whatDayIsIt() {
  ...
  let arrGiorni = ['Domenica', 'Lunedì', 'Martedì', 'Mercoledì', 'Giovedì', 'Venerdì', 'Sabato'];
...
let dataInserire = new Date(2023, 10, 10);
  

console.log('Esercizio 9');
console.log(`Sono trascorsi ${howManyDays(dataInserire)} giorni`) 
...
function isTodayMyBirthday() {
  ...
  let giornoOggi = dataOggi.getDate();
  let meseOggi = dataOggi.getMonth() + 1;

  if(giornoCompl === giornoOggi && meseCompl === meseOggi) {
    return true;
  }else {
    return false;
  }
...
console.log('Esercizio 11:', deleteProp(oggetto, 'colore'));
...
console.log('I film contenuti nell\'array, sono:', countMovies(movies));
...
function searchAndDivide(arr, str) {
  ...
  let match = arr.filter((el) => el.Title.includes(str));
  oggetto.match.push(match);
  let unMatch = arr.filter((el) => !el.Title.includes(str));
  oggetto.unmatch.push(unMatch);
...
function removeIndex(arr, num) {
  let numMovie = arr.filter((el, index) => index !== num);
  return numMovie;
}
